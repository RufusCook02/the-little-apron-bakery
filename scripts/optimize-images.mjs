...
async function optimizeFile(filePath) {
  ...
  if (originalSize < SKIP_BELOW_BYTES) {
    return { filePath, skipped: true, reason: "under size threshold", originalSize, newSize: originalSize };
  }
...
async function main() {
  ...
  for (const filePath of imageFiles) {
    ...
    if (result.skipped) {
      console.log(`  skip  ${rel}  (${formatBytes(result.originalSize)}) - ${result.reason}`);
    } else {
      changedCount += 1;
